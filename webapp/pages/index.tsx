import { GetServerSideProps, NextPage } from "next";

import Head from "next/head";
import { X as XIcon } from "react-feather";
import { useState } from "react";

export type DocumentResultType = {
  title: string;
  text: string;
  summary?: string;
  page_spans: SpanType[];
  concept_spans: SpanType[];
  sentence_spans: SpanType[];
  id: string;
  concepts: string[];
  type: string;
};
export type SpanType = {
  start_index: number;
  end_index: number;
  identifier?: string;
  type?: string;
};

type Props = {
  queryParams: { searchTerms?: string; n?: number };
  results?: { [key: string]: DocumentResultType };
  total?: number;
};

const Search: NextPage<Props> = (props) => {
  const [searchTerms, setSearchTerms] = useState(props.queryParams.searchTerms);
  const [results, setResults] = useState(props.results);

  return (
    <div>
      <Head>
        <link
          rel="icon"
          href={`data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y=".9em" font-size="90">💼</text></svg>`}
        />
        <title>Tribunals search</title>
        <meta name="description" content="Search for tribunal documents" />
      </Head>
      <main className="min-h-screen">
        <form
          className="border-gray-500 flex w-full divide-x-2 border-2 "
          autoComplete="off"
        >
          <div className="relative mx-auto flex w-full items-center">
            <input
              className="w-full bg-transparent py-2 pl-4 pr-8 text-lg placeholder:text-gray focus:outline-none"
              type="text"
              name="query"
              value={searchTerms}
              autoComplete="off"
              placeholder="What are you looking for?"
              onChange={(e) => setSearchTerms(e.target.value)}
              required
            ></input>
            <button
              type="reset"
              className="absolute right-0 mr-2"
              onClick={() => {
                setSearchTerms("");
                setResults({});
              }}
            >
              <XIcon className="w-6" />
            </button>
          </div>
          <button
            type="submit"
            className="py-2 px-3 text-lg"
            onClick={() => {
              if (searchTerms) {
                window.location.href = `/?query=${searchTerms}`;
              }
            }}
          >
            Search
          </button>
        </form>

        {results && (
          <p className="pt-2 text-sm">
            found {props.total} result{props.total == 1 ? "" : "s"}
          </p>
        )}

        <div className="pt-4">
          <ul>
            {results &&
              Object.keys(results).map((key) => (
                <li key={key} className="py-4">
                  <h2 className="text-lg font-bold">{results[key].title}</h2>
                  <p>{results[key].summary}</p>
                </li>
              ))}
          </ul>
        </div>
      </main>
    </div>
  );
};

export const getServerSideProps: GetServerSideProps = async ({ query }) => {
  if (!query.query) {
    return {
      props: {
        queryParams: { searchTerms: "" },
        results: null,
        total: null,
        took: null,
      },
    };
  } else {
    const searchResponse = await fetch(
      `http://localhost:8000/documents?query=${encodeURIComponent(
        query.query as string
      )}`
    ).then((res) => res.json());
    return {
      props: {
        queryParams: { searchTerms: query.query },
        results: searchResponse.results,
        total: searchResponse.totalResults,
      },
    };
  }
};

export default Search;
